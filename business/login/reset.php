...
if( isset($_GET['email']) && !empty($_GET['email']) AND isset($_GET['shopid']) && !empty($_GET['shopid']) )
{
    $email = $connect->escape_string($_GET['email']); 
    $shopid = $connect->escape_string($_GET['shopid']); 

    // Make sure user email with matching hash exist
    $result = $connect->query("SELECT * FROM shops WHERE `ShopE-mail`='$email' AND `ShopID`='$shopid'");
