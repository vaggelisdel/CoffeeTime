...
{   
    ...
    $result = $connect->query("SELECT * FROM shops WHERE `ShopE-mail` ='$email'");

    if ( $result->num_rows == 0 ) // User doesn't exist
    { 
        $_SESSION['message'] = "Shop with that email doesn't exist!";
        header("location: error.php");
    }
    else { // User exists (num_rows != 0)

        $shop = $result->fetch_assoc(); // $user becomes array with user data
		
        $email = $shop['ShopE-mail'];
        ...
        http://'.$domain.'/business/login/reset.php?email='.$email.'&shopid='.$shopid;
