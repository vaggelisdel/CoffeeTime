...
<?php  
 ...
 $query ="SELECT * FROM admins, adminimages WHERE admins.AdminID = adminimages.AdminID ORDER BY admins.RegisterDate DESC";  
    ...
    <section class="sidebar">
      ...
      <ul class="sidebar-menu">
		...
		<li class="active treeview">
          ...
          <ul class="treeview-menu">
            <li class="active"><a href="admins.php"><i class="fa fa-circle-o"></i>Προβολή Όλων</a></li>
	...
	?>
		   <div class="box box-info">
            <div class="box-header with-border">
              <h3 class="box-title">Διαχειριστές</h3>
			...
			<table id="employee_data" class="table table-striped table-bordered">  
                          ...
                          {  
							  ...
							  $result3 = $connect->query("SELECT count(*) as createdshops FROM shops WHERE `CreatedBy` ='$username'");
							  $createdshops = $result3->fetch_assoc();
                               echo '  
							   							   
                               <tr>  
							   <td><a href="'.dirname($_SERVER['REQUEST_URI']).'/editadmins.php?editadmin=&adminid1='.$row["AdminID"].'&username='.$row["UserName"].'"><img src="../../'.$row["AdminImage"].'" style="width: 35px;height: 35px;border-radius:50%;object-fit: cover;"></a></td> 
									<td>'.$row["AdminID"].'</td> 
                                    <td>'.$row["UserName"].'</td>  
                                    <td>'.$row["E-mail"].'</td>  
                                    <td>'.$row["RegisterDate"].'</td>  
                                    <td>'.$row["LastAdminLogin"].'</td>
									<td>'.$createdshops["createdshops"].'</td>									
                                    <td>'; if ($row["Active"] == '1'){ 
					echo '<span class="label label-success">Ενεργός</span>'; 
							...
							echo '<span class="label label-danger">Ανενεργός</span>';
									...
									<td><center><a href="'.dirname($_SERVER['REQUEST_URI']).'/editadmins.php?editadmin=&adminid1='.$row["AdminID"].'&username='.$row["UserName"].'"><span class="label label-primary">Επεξεργασία</span></a></center></td>
                               </tr>  
                               ';  
                          }  
                          ?>  
