...
if(isset($_GET['editinfo']) || isset($_GET['shopid'])) {
...
<input type="hidden" id="deleteshop_ckeck" name="deleteshop_ckeck" value="0">
		<button type="button" name="deletebtn" class="deleteshop delete_btns">Διαγραφή αυτού του μαγαζιού</button>
		
		
		
					<div class="toggle_shop active_shop">
			<span class="label_toggle1">Ενεργοποίηση καταστήματος: </span>
			<?php
				if($shop7['Active'] == "0"){
					?>
					<label class="switch">
	...
	?>
			...
			<div class="saves">
				<div class="inlinedisplay">
					...
					<input type="hidden" name="active_status" id="active_status" value="<?= $shop7['Active']; ?>">
...
<label class="btn btn-primary btn-upload" for="inputImage" title="Upload image file">
	<input type="file" class="sr-only" id="inputImage" onchange="fileuploaded()" name="file" accept=".jpg,.jpeg,.png,.gif,.bmp,.tiff">
	<span class="docs-tooltip upbtn" data-toggle="tooltip" data-animation="false" title="Import image with Blob URLs">
		<span class="fa fa-upload"></span> Upload File
	</span>
</label>		
<div class="modal fade docs-cropped" id="cropbox" aria-hidden="true" role="dialog" tabindex="-1">
						  ...
						  <button type="button" class="btn btn-primary" data-method="scaleY" data-option="-1" title="Flip Vertical">
							<span class="docs-tooltip" data-toggle="tooltip" data-animation="false" title="$().cropper(&quot;scaleY&quot;, -1)">
							  ...
							  <div class="btn-group btn-group-crop">
								  <button type="button" name="uploadbtn" id="upload" class="btn btn-primary" data-method="getCroppedCanvas" data-option="{ &quot;maxWidth&quot;: 4096, &quot;maxHeight&quot;: 4096 }">
								  ...
								  <input type="hidden" name="getimageurl" id="getimageurl" value=""/>
								  <input type="hidden" name="getimagename" id="getimagename" value=""/>
      ...
      <div id="map"></div>
						...
						<div class="card">
                            <div class="card-block form-material">
									...
									<button type="button" class="btn btn-danger" id="insert_img_btn" name="insert_img_btn">Μεταφόρτωση εικόνας</button>
								...
								<input type="hidden" name="shopid" value="<?= $shop7['ShopID']; ?>">
									...
									<div class="form-group col-sm-6">
                                        <label>Latitude</label>
                                            <input type="text" name="maplat" id="lat" value="<?= $shop7['Maps_Lat']; ?>" class="form-control form-control-line">
                                    </div>
									<div class="form-group col-sm-6">
                                        <label>Longitude</label>
                                            <input type="text" name="maplng" id="lng" value="<?= $shop7['Maps_Lng']; ?>" class="form-control form-control-line">
                                    </div>
                                </div>
                            </div>
                        </div>
						...
						<div class="card">
                            <div class="card-block">
                                <div class="form-horizontal form-material">
                                    ...
                                    <div class="form-group col-sm-6">
                                        <label>Κωδικός</label>
                                        <input type="password" name="password" placeholder="Εισάγετε νέο κωδικό" class="form-control form-control-line" name="example-email" id="example-email">
                                    </div>
                                </div>
                            </div>
                        </div>
...
<table>
  ...
  <tr>
    ...
    <td><center><input type="text" name="facebook" id="facebook" value="<?= $shop12['FacebookURL']; ?>"></center></td>
  ...
  <tr>
    ...
    <td><center><input type="text" name="twitter" value="<?= $shop12['TwitterURL']; ?>"></center></td>
  ...
  <tr>
    ...
    <td><center><input type="text" name="instagram" value="<?= $shop12['InstagramURL']; ?>"></center></td>
  ...
  <tr>
    <td>https://www.</td>
    <td><center><input type="text" name="website" value="<?= $shop12['Website']; ?>"></center></td>
  ...
  <tr>
    <td>https://www.</td>
    <td><center><input type="text" name="deliverysite" value="<?= $shop12['DeliverySite']; ?>"></center></td>
    ...
    <tr>
    <td>Κωδικός WiFi</td>
    <td><center><input type="text" name="wifipassword" value="<?= $shop7['WiFiPassword']; ?>"></center></td>
  ...
  <tr>
    <td>E-mail <span id="availabilityEmail"></span></td>
    <td><center><input type="text" id="email" name="shopemail" value="<?= $shop7['ShopE-mail']; ?>"></center></td>
  ...
  <tr>
    ...
    <td><center><input type="text" name="contactphone" value="<?= $shop7['ContactPhone']; ?>"></center></td>
  ...
  <tr>
    <td>Τηλέφωνο επικοινωνίας 2</td>
    <td><center><input type="text" name="contactphone2" value="<?= $shop7['ContactPhone2']; ?>"></center></td>
...
$(document).on('click', '#upload', function(){
	var getimageurl = encodeURIComponent($("#getimageurl").val());
	var getimagename = $("#getimagename").val();
	var cur_action = "UploadLogo";
	
	$.ajax(
            {
                url:'cropper/upload_logo.php',
                type:'POST',
                data:'getimageurl=' + getimageurl + '&getimagename=' + getimagename + '&action=' + cur_action,
...
<script>  
 $(document).ready(function(){  
   $('#username').blur(function(){
     ...
     $.ajax({
      url:'check_shop_onedit.php',
      method:"POST",
      data:{user_name:username, shop_id:shopid},
      success:function(data)
      {
       if(data != '0')
	   {
 ...
 $(document).ready(function(){  
   $('#email').blur(function(){
     ...
     $.ajax({
      url:'check_shop_onedit.php',
      method:"POST",
      data:{e_mail:email, shop_id:shopid},
      success:function(data)
      {
       if(data != '0')
       {
        $('#availabilityEmail').html('<span class="text-danger">Μη διαθέσιμο E-mail</span>');
		$('#editinfobtn').attr("disabled", true);
	   }
       else
       {
        $('#availabilityEmail').html('<span class="text-success">Διαθέσιμο E-mail</span>');
		$('#editinfobtn').attr("disabled", false);
       }
      }
     })
...
$(document).on('click', '.deleteshop', function(){
	...
	if (confirm("Διαγραφή αυτού του καταστήματος;")) {
		
		document.getElementById("deleteshop_ckeck").value = "1";
...
$(document).on('click', '.delete', function(){
  var itemid = $(this).attr("value"); 
  if(confirm("Διαγραφή του προϊόντος με κωδικό: "+itemid+"?"))
  {
   var action = "Delete";
...
$(document).on('click', '.duplicate', function(){
  var itemid = $(this).attr("value"); 
  if(confirm("Δημιουργία διπλότυπου του προϊόντος με κωδικό: "+itemid+"?"))
  {
 ...
 $(document).ready(function(){  
   $('#categoryname').blur(function(){
	 ...
	 if ((categoryname == 'Καφέδες') || (categoryname == 'Χυμοί') || (categoryname == 'Μπύρες') || (categoryname == 'Ποτά')){
		...
		$('#availability2').html('<span class="text-danger">Μη διαθέσιμη κατηγορία</span>');
  ...
  $(document).ready(function(){  
   $('#updatedcateg').blur(function(){
	 ...
	 if ((updatedcateg == 'Καφέδες') || (updatedcateg == 'Χυμοί') || (updatedcateg == 'Μπύρες') || (updatedcateg == 'Ποτά')){
		...
		$('#availability3').html('<span class="text-danger">Μη διαθέσιμη κατηγορία</span>');
